...
def leer_cv_pdf(path: str) -> str:
    """Lee un archivo .pdf y retorna todo su texto concatenado."""
...
def extraer_nombre_cv(texto: str, cliente_groq: Groq, modelo_llm: str) -> str:
    """Extrae el nombre del candidato analizando el contenido del CV con un LLM."""
    prompt = f"""Eres un asistente especializado en leer Currículums. 
Tu única tarea es extraer el NOMBRE Y APELLIDO principal de la persona dueña de este CV.
El nombre suele estar al principio del documento.

Responde ÚNICAMENTE con el nombre y apellido. Sin saludos, sin explicaciones, sin comillas.
Ejemplo de respuesta correcta: Juan Perez
...
"""
    try:
        ...
        # Limpiar posibles saltos de linea o markdown extra
        return nombre.split('\n')[0].strip()
    except Exception as e:
        st.warning(f"No se pudo extraer el nombre con LLM ({e}).")
        return "Desconocido"

def chunking_semantico_local(texto: str) -> List[Dict]:
    """
    Usa el RecursiveCharacterTextSplitter de LangChain para dividir el texto
    de forma predecible y lógica (por párrafos y luego oraciones), sin consumir tokens.
    """
...
def generar_respuesta_rag(
    ...
    if agent_name:
        system_prompt = f"""Eres el agente especializado en representar a {agent_name}. Tienes acceso al contenido de su CV.
    
REGLAS ESTRICTAS:
- Responde de forma directa a la pregunta basándote ESTRICTAMENTE en la información de {agent_name}.
- Asume que cualquier mención a "mi", "tu" o "su" (ej. "mi email", "tu teléfono", "su experiencia") se refiere SIEMPRE a la información de {agent_name}. El usuario puede estar hablando en primera o segunda persona, pero los datos a buscar son los de {agent_name}.
- Si el contexto no contiene información sobre la pregunta, simplemente di: "No tengo información sobre eso en el CV." 
- NO inventes datos ni asumas información de otras personas.
- Ten en cuenta el historial para entender el contexto de la pregunta, pero SOLO responde por {agent_name}.
- Responde en primera persona ("Soy", "Tengo", "Mi teléfono es") asumiendo la identidad de {agent_name}.

CONTEXTO DEL CV DE {agent_name}:
{contexto_texto}"""
    else:
...
{contexto_texto}"""
    ...
    if historial:
        if agent_name:
            # Para sub-agentes, pasamos el historial como contexto para no contaminar su identidad "Assistant"
            historia_str = "HISTORIAL DE CONVERSACIÓN RECIENTE:\n"
            for msg in historial[-3:]:
                historia_str += f"- Usuario: {msg['pregunta']}\n- Respuesta Global: {msg['respuesta']}\n"
            messages.append({"role": "user", "content": historia_str})
        else:
            for msg in historial[-3:]:
...
def main():
    ...
    with st.sidebar:
        ...
        data_dir = st.text_input(
            "Directorio de archivos (.docx, .pdf)",
            value="data",
        )
    ...
    if "perfiles_disponibles" not in st.session_state:
        st.session_state.perfiles_disponibles = []
    ...
    if procesar:
        ...
        archivos_a_procesar = []
        if os.path.isfile(data_dir) and (data_dir.endswith(".docx") or data_dir.endswith(".pdf")):
            archivos_a_procesar.append(data_dir)
        elif os.path.isdir(data_dir):
            for f in os.listdir(data_dir):
                if f.endswith(".docx") or f.endswith(".pdf"):
                    archivos_a_procesar.append(os.path.join(data_dir, f))
        else:
            st.error(f"La ruta ingresada no es un directorio válido ni un archivo .docx/.pdf: {data_dir}")
            st.stop()

        if not archivos_a_procesar:
            st.error(f"No se encontraron archivos .docx o .pdf en {data_dir}")
            st.stop()
        ...
        with st.status("Procesando CVs...", expanded=True) as status:
            ...
            st.write(f"Paso 1 y 2: Leyendo y chunking {len(archivos_a_procesar)} CVs...")
            for path in archivos_a_procesar:
                arc = os.path.basename(path)
                st.write(f"Procesando {arc}...")
                
                if path.endswith(".docx"):
                    texto_cv = leer_cv_docx(path)
                else:
                    texto_cv = leer_cv_pdf(path)
                    
                st.session_state.cv_texto += f"\n\n--- {arc} ---\n" + texto_cv
                
                # Extraer nombre usando LLM a partir del contenido
                person_name = extraer_nombre_cv(texto_cv, cliente_groq, modelo_llm)
                st.write(f"  -> Perfil identificado: **{person_name}**")
                
                try:
                    chunks = chunking_semantico_local(texto_cv)
                except Exception as e:
                    st.warning(f"Chunking semantico fallo ({e}). Usando chunking naive.")
                    chunks = dividir_en_chunks_naive(texto_cv, tamano=tamano_chunk, overlap=overlap_chunk)
                
                for c in chunks:
                    c['person_name'] = person_name
                    # Make ID unique across people
                    c['id'] = f"{person_name.replace(' ', '_')}_{c['id']}"
                
                all_chunks.extend(chunks)

            st.write(f"  {len(all_chunks)} chunks totales generados.")
            st.session_state.chunks = all_chunks

            # Paso 3: Embeddings
            st.write("Paso 3: Generando embeddings con all-MiniLM-L6-v2...")
            modelo_emb = cargar_modelo_embeddings()
            textos_chunks = [c["texto"] for c in all_chunks]
            ...
            subir_chunks_a_pinecone(indice, all_chunks, embeddings)
            ...
            st.session_state.perfiles_disponibles = list(set([c.get("person_name", "Desconocido") for c in all_chunks]))
    ...
    if st.session_state.indice_listo:
        ...
        if pregunta := st.chat_input("Ej: Donde estudio? Que tecnologias maneja?"):
            ...
            with st.chat_message("assistant"):
                    ...
                    estado_inicial = {
                        "query": pregunta,
                        "chat_history": st.session_state.historial_chat,
                        "names_to_query": [],
                        "agent_responses": [],
                        "final_response": "",
                        "retrieved_chunks": [],
                        ...
                        "modelo_llm": st.session_state.get("modelo_llm", modelo_llm),
                        "top_k": top_k,
                        "available_profiles": st.session_state.perfiles_disponibles,
                        "revision_count": 0,
                        "reviewer_feedback": ""
                    }
                    
                    resultado = app_graph.invoke(estado_inicial)
                    respuesta = resultado["final_response"]
                    chunks_relevantes = resultado.get("retrieved_chunks", [])
                    names_queried = resultado.get("names_to_query", [])

                    st.markdown(respuesta)

                    with st.expander(f"Contexto (Agentes consultados: {', '.join(names_queried)})"):
